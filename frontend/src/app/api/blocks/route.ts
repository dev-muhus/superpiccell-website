  ...
  try {
    ...
    if (cursor) {
      ...
      if (sortDirection === asc) {
        conditions = and(
          conditions,
          gt(blocks.created_at, cursorDate)
        );
      } else {
        conditions = and(
          conditions,
          lt(blocks.created_at, cursorDate)
        );
      }
    }
    ...
    .where(conditions)
    .orderBy(sortDirection(blocks.created_at))
    .limit(limit + 1); // 次ページがあるかを判断するため、limit+1件取得

    // 次ページの有無を判定
    const hasNextPage = results.length > limit;
    const blocks_list = hasNextPage ? results.slice(0, limit) : results;

    // 次ページのカーソルを生成
    let nextCursor = null;
    if (hasNextPage && blocks_list.length > 0) {
      const lastItem = blocks_list[blocks_list.length - 1];
      const cursorData = {
    ...
    console.error('ブロック一覧取得中にエラーが発生しました:', error);
    return NextResponse.json(
